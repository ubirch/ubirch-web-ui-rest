...
object DeviceFactory extends LazyLogging {
  ...
  protected[keycloak] def createDeviceAdmin(device: AddDevice, provider: String)(implicit realmName: String): Device = {
  ...
  protected[keycloak] def createDevice(device: AddDevice, owner: User)(implicit realmName: String): Device = {
    Util.stopIfHwdeviceidIsNotUUID(device.hwDeviceId)
    Util.stopIfMemberAlreadyExist(device.hwDeviceId)
