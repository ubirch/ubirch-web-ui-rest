...
object MemberFactory extends LazyLogging {
  ...
  def getByFirstName(name: String, namingConvention: String = "Name", brief: Boolean = false)(implicit realmName: String): UserRepresentation = {
    logger.debug("name: " + name)

    if (name.isEmpty) {
      throw BadRequestException(s"$namingConvention should not be empty")
    } else {
      QuickActions.quickSearchFirstNameStrict(name, brief)
    }
  }
